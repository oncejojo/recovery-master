    ...
    private static Object mapToBean(Map<String, String> map) {
        ...
        try {
            if (type.equals(VxMsgEnum.EVENT.val)) { // 事件消息
                ...
                if (evenType.equals(VxEventEnum.CLICK.val)){
                    EventMenuClickMsg clickMsg = new EventMenuClickMsg();
                    mapToBean(clickMsg,map);
                    return clickMsg;
                }else if (evenType.equals(VxEventEnum.SUBSCRIBE.val)){
                    EventMsgBase evenMsgBase = new EventMsgBase();
                    mapToBean(evenMsgBase,map);
                    return evenMsgBase;
                }else if (evenType.equals(VxEventEnum.UNSUBSCRIBE.val)){
                    EventMsgBase evenMsgBase = new EventMsgBase();
                    mapToBean(evenMsgBase,map);
                    return evenMsgBase;
                }else if (evenType.equals(VxEventEnum.VIEW.val)){
                    EventViewMsg viewMsg = new EventViewMsg();
                    mapToBean(viewMsg,map);
                    return viewMsg;
                }else if (evenType.equals(VxEventEnum.SCAN.val)){
                    EventScanMsg scanMsg = new EventScanMsg();
                    mapToBean(scanMsg,map);
                    return scanMsg;
                }else if (evenType.equals(VxEventEnum.LOCATION.val)){
